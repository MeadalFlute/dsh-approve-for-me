...
export function apply(ctx, config = {}) {
     ...
     */
    ctx.effect(() => loader.registry.tools.patchAll((def) => patchTool(def, grantMode), {
    ...
    const appendReviewLogFailure = (req, error) => {
        const session = req.agent.session;
        const approvalId = pendingApprovalIdFor(session, req);
        if (approvalId === undefined)
            return;
     ...
     */
    const injectReviewNotice = (agent, summary, text, force = false) => {
        if (!force && !reviewNotify)
            return;
        try {
        ...
        if (state.mode === "review") {
            ...
            try {
                ...
                if (assessment.outcome === "deny" && (config.reviewDenyFallback ?? "deny") === "ask") {
                    // Hand the decision to the human, carrying the AI's rationale
                    // so the approval panel can show it as a second line. The
                    // host UI panel (dsh-client-ui-approval) reads `req.reviewContext`
                    // and renders it under the request reason; the original request
                    // keeps its own `reason` untouched for the first line.
                    appendReviewLog(req, assessment);
                    const reviewText = [
                        `AI 审查拒绝（risk: ${assessment.riskLevel}，authorization: ${assessment.userAuthorization}）`,
                        assessment.rationale,
                    ].join("\n");
                    req.reviewContext = reviewText;
                    const approvalId = pendingApprovalIdFor(session, req);
                    if (approvalId !== undefined) {
                        humanPending.set(String(approvalId), {
                            agent: req.agent,
                            reviewText,
                            toolLabel,
                        });
                    }
                    ctx.logger.info(`[approve-for-me] reviewer denied ${req.toolName} (risk=${assessment.riskLevel}, auth=${assessment.userAuthorization}); handing to human with rationale`);
                    return next();
                }
                ...
                if (decision === "rejected" || reviewNotify) {
                    // Rejections always surface the AI's rationale (checked by the
                    // user to decide whether to retry or override); approvals stay
                ...
                if ((config.reviewFallback ?? "deny") === "ask") {
                    // Handing control to the human: tell them WHY the reviewer
                    // could not decide, so the prompt is not a blind ask.
    ...
    ctx.effect(() => loader.registry.permissionPresets.define(presetName, {
    ...
    if (strictPresetName !== "" && strictPresetName !== presetName) {
        ctx.effect(() => loader.registry.permissionPresets.define(strictPresetName, {
