...
## ⭐ This fork: fixes and additions (relative to upstream 0.4.1)

> The upstream main branch (`dsh-plugins/dsh-approve-for-me`, checked 2026-09) does **not** contain any of
> the following. Please see the Chinese `README.md` for full details.

- **Fix 1 – preset detection**: upstream called `loader.registry.permissionPresets.effective()`, which
  depends on an `effectivePermissionPreset` export the host `dsh-permission-presets` does **not** provide,
  so selecting "Approve For Me" in the GUI never registered (approvals always fell through to the popup).
  This fork queries the live `permissionPresets` service (`current(session)`) with a local event-fold
  fallback instead.
- **Fix 2 – reviewer message construction**: upstream used `loader.llm.createUserMessage`, which throws
  when dsh-loader cannot ESM-import `@deepseek-ai/dsh-llm` (profiles have no `@deepseek-ai/*` dependency).
  This fork adds `makeUserMessage()` with a local fallback.
- **New – `reviewDenyFallback: "ask"`**: when the AI reviewer denies, hand the decision back to the human
  with the AI's rationale shown as a second line in the approval dialog, then report the human's verdict
  back to the agent.
- **Host bundle patch (required for the second panel line)**: see [`patches/`](patches/) —
  `apply-ui-approval.ps1` (idempotent, re-run after every dsh upgrade) plus the readable
  `dsh-client-ui-approval.patch`.
