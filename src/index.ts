...
interface ToolDefinition {
  name?: string;
  parameters?: { sandbox_permissions?: SandboxPermissionSpec };
...
/** The permission-preset table exposed by `permissionPresets`. */
interface PermissionPresetTable {
 ...
 */
function patchTool(def: ToolDefinition, grantMode: GrantMode): void {
  if (def === null || typeof def !== "object" || def[PATCHED]) return;
  const spec = def.parameters?.sandbox_permissions;
  if (spec === undefined || typeof spec !== "object" || !Array.isArray(spec.enum)) return;
...
export function apply(ctx: PluginContext, config: ApproveForMeConfig = {}): void {
  ...
  const approve: CompiledPatterns = compilePatterns(config.approve ?? []);
  const deny: CompiledPatterns = compilePatterns(config.deny ?? []);
  ...
  const rememberStrictEscalation = (session: Session, callId: string | undefined, args: unknown): void => {
    if (callId === undefined || typeof args !== "object" || args === null || typeof (args as Record<string, unknown>).sandbox_permissions !== "string") return;
  ...
  const consumeStrictEscalation = (session: Session, callId: string | undefined): boolean => {
    if (callId === undefined) return false;
    const callIds = strictApprovedEscalations.get(session);
    if (callIds === undefined || !callIds.delete(callId)) return false;
    if (callIds.size === 0) strictApprovedEscalations.delete(session);
    return true;
  };
  ...
  const commandTextFor = (session: Session, callId: string | undefined): string => {
    ...
    for (let index = events.length - 1; index >= 0; index -= 1) {
      ...
      try {
        const parsed = JSON.parse(event.data.arguments) as { command?: unknown };
   ...
   */
  const pendingApprovalIdFor = (session: Session, req: ApprovalRequest): string | undefined => {
    const decided = new Set<string>();
    const candidates: string[] = [];
    for (let index = session.events.length - 1; index >= 0; index -= 1) {
      const event = session.events[index];
      if (event.type === "approval/decided") {
        decided.add(event.data?.id as string);
        continue;
      }
      if (
        event.type === "approval/asked" &&
        !decided.has(event.data?.id as string) &&
        (event.data?.callId ?? null) === (req.callId ?? null)
      ) {
        candidates.push(event.data?.id as string);
        if (req.callId !== undefined) return event.data?.id as string;
      }
    }
  ...
  const appendReviewLog = (req: ApprovalRequest, assessment: GuardianAssessment): void => {
  ...
  const appendStrictReviewStarted = (exec: ToolExec): void => {
    const session = exec.agent?.session;
    if (session === undefined) return;
    try {
   ...
   */
  const runReview = async (req: ApprovalRequest): Promise<GuardianAssessment> => {
    const llm = ctx.get<LlmService>("llm");
    if (llm === undefined) throw new Error("review mode requires ctx.llm (no LLM service mounted)");
    ...
    for (let index = session.events.length - 1; index >= 0; index -= 1) {
      ...
      if (event.type === "request/context") {
        inheritedProvider = event.data?.provider as string | undefined;
        inheritedModel = event.data?.model as string | undefined;
        break;
      }
    }
    ...
    if (!provider || !model) {
      throw new Error(
        "review mode requires reviewProvider/reviewModel (or a session with a recorded model route); nothing to review with",
      );
    }
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
      try {
        ...
        if (req.signal?.aborted) throw error; // cancellation: do not retry
        lastError = error;
      }
    }
  ...
  const collectReviewText = async (
    ...
    try {
      ...
      if (timedOut) throw new Error(`reviewer timed out after ${options.timeoutMs}ms`);
  ...
  ctx.on(
    "tools/pre-execute",
    async (exec: ToolExec, next: () => Promise<unknown>) => {
      if (exec.agent === undefined || exec.signal?.aborted === true) return next();
      const session = exec.agent.session;
      const state = resolveState(session);
      if (state.mode !== "review" || !state.strictActive) return next();
      ...
      const justification =
        typeof args === "object" && args !== null && typeof args.justification === "string"
          ? args.justification
          : "";
      let assessment: GuardianAssessment | undefined;
      try {
      ...
      if (assessment.outcome === "allow") {
        ...
        ctx.logger.info(
          `[approve-for-me] strict reviewer approved ${exec.name} (risk=${assessment.riskLevel}, auth=${assessment.userAuthorization}, rationale=${assessment.rationale})`,
        );
        return next();
      }
      ...
      ctx.logger.info(
        `[approve-for-me] strict reviewer denied ${exec.name} (risk=${assessment.riskLevel}, auth=${assessment.userAuthorization}, rationale=${assessment.rationale})`,
      );
  ...
  ctx.on(
    "approval/request",
    async (req: ApprovalRequest, next: () => Promise<string>) => {
      ...
      if (state.mode === "review" && !state.presetActive) return next();
      ...
      if (state.mode === "review") {
        ...
        try {
          ...
          if (req.signal?.aborted) {
            if (reviewNotify) injectReviewNotice(req.agent, "⏹ 审查已取消（请求中止）");
            return "cancelled";
          }
